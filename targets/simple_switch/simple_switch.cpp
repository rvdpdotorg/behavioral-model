...
class SimpleSwitch::InputArbitration {
 public:
  ...
  InputArbitration(size_t capacity_hi, size_t capacity_lo)
...
SimpleSwitch::SimpleSwitch(port_t max_port, bool enable_swap)
  : Switch(enable_swap),
    max_port(max_port),
    input_arbitration(new InputArbitration(
        1024 /* normal capacity */, 1024 /* resubmit capacity */)),
...
SimpleSwitch::receive_(port_t port_num, const char *buffer, int len) {
  ...
  input_arbitration->push_front(
      InputArbitration::PacketType::NORMAL, std::move(packet));
  return 0;
}
...
SimpleSwitch::~SimpleSwitch() {
  input_arbitration->push_front(
      InputArbitration::PacketType::SENTINEL, nullptr);
  for (size_t i = 0; i < nb_egress_threads; i++) {
...
SimpleSwitch::ingress_thread() {
  ...
  while (1) {
    std::unique_ptr<Packet> packet;
    input_arbitration->pop_back(&packet);
    if (packet == nullptr) break;
    ...
    if (phv->has_field("intrinsic_metadata.resubmit_flag")) {
      ...
      if (f_resubmit.get_int()) {
        ...
        copy_field_list_and_set_type(packet, packet_copy,
                                     ...
                                     field_list_id);
        input_arbitration->push_front(
            InputArbitration::PacketType::RESUBMIT, std::move(packet_copy));
        continue;
      }
    }
  ...
  while (1) {
    ...
    if (phv->has_field("intrinsic_metadata.recirculate_flag")) {
      ...
      if (f_recirc.get_int()) {
        ...
        input_arbitration->push_front(
            InputArbitration::PacketType::RECIRCULATE, std::move(packet_copy));
        continue;
      }
    }
